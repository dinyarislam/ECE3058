...
void cachesim_init(int _block_size, int _cache_size, int _ways) {
    ...
    for (int j = 0; j < num_sets; j++) {
        cache[j].size = ways;
        cache[j].stack = init_lru_stack(ways); // function imported from lrustack.h for initialization
        cache[j].blocks = (cache_block_t*)malloc(ways * sizeof(cache_block_t));
    }
...
void cachesim_access(addr_t physical_addr, int access_type) {
    ...
    // format: tag | index | offset/num_sets
    int index = (physical_addr / block_size) % num_sets;
    int tag = (physical_addr / block_size) / num_sets;
    accesses++; // increment access counter
    for (int j = 0; j < cache[index].size; j++) {
       /*
       if cache miss and set not full...
       */
       if (cache[index].blocks[j].valid ==  0) {
            cache[index].blocks[j].tag = tag;
            cache[index].blocks[j].valid = 1;
            if (access_type == MEMWRITE) {
                cache[index].blocks[j].dirty = 1;
            } else if (access_type == MEMREAD || access_type == IFETCH) {
                cache[index].blocks[j].dirty = 0;
            }
            lru_stack_set_mru(cache[index].stack, j); // function imported from lrustack.h
        ...
        if cache hit...
        */
        else if (cache[index].blocks[j].valid == 1) {
            if (cache[index].blocks[j].tag == tag) {
                if (access_type == MEMWRITE) {
                    cache[index].blocks[j].dirty = 1;
                }
                lru_stack_set_mru(cache[index].stack, j); // function imported from lrustack.h
    ...
    /*
    If cache miss and set full...
    */
    int lru_i = lru_stack_get_lru(cache[index].stack); // function imported from lrustack.h
    // if block is dirty
    if (cache[index].blocks[lru_i].dirty == 1 && cache[index].blocks[lru_i].valid == 1) {
        writebacks++;
    }
    cache[index].blocks[lru_i].valid = 1;
    cache[index].blocks[lru_i].tag = tag;
    // update dirty bit
    if (access_type == MEMWRITE) {
        cache[index].blocks[lru_i].dirty = 1;
    } else if (access_type == MEMREAD || access_type == IFETCH) {
        cache[index].blocks[lru_i].dirty = 0;
    }
    lru_stack_set_mru(cache[index].stack, lru_i); // function imported from lrustack.h
    misses++; // increment miss counter
...
void cachesim_cleanup() {
    ...
    for (int j = 0; j < num_sets; j++) {
      free(cache[j].blocks);
      lru_stack_cleanup(cache[j].stack); // function imported from lrustack.h
    }
    free(cache);
