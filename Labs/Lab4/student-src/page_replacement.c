...
pfn_t free_frame(void) {
    ...
    {
        pfn_t ptbr_idx = frame_table[victim_pfn].process->saved_ptbr;
        pte_t* page_table = (pte_t*)(mem + (ptbr_idx * PAGE_SIZE));
        pte_t* page_table_entry = (pte_t*) (page_table + frame_table[victim_pfn].vpn);
        if (page_table_entry->dirty)
        {
            void* source = (void*)(mem + victim_pfn * PAGE_SIZE);
            swap_write(page_table_entry, source);
